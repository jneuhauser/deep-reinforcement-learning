...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        state_action_values = self.qnetwork_local(states).gather(dim=1, index=actions)

        # Get max predicted Q values (for next states) from target model
        # Compute V(s_{t+1}) for all next states.
        # Expected values of actions for non_final_next_states are computed based
        # on the "older" qnetwork_target; selecting their best reward with max(dim=1)[0].
        next_state_values = self.qnetwork_target(next_states).max(dim=1)[0].detach().unsqueeze(dim=1)
        # Compute the expected Q values
        # Handle final states with * (1 - done) = 0 if done = 1
